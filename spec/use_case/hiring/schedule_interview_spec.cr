...
Spectator.describe Hiring::ScheduleInterview do
  ...
  let(candidate) { DTO::Candidate.new(candidate_id, "Arthur", [skill_java, skill_python], availability_2) }
  let(candidate_id) { 0 }
  ...
  let(skillfull_recruiter) do
    DTO::Recruiter.new(1, "Mathieu", [skill_java, skill_python], [availability_1, availability_2])
  end

  let(java_recruiter) do
    DTO::Recruiter.new(2, "Vincent", [skill_java], [availability_1, availability_3])
  end

  let(python_recruiter) do
    DTO::Recruiter.new(3, "Thomas", [skill_python], [availability_2, availability_3])
  end
